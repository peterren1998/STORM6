...
class SyringePumpControl(PumpControl):
    ...
    def createGUI(self):
        ...
        self.init_button = QtWidgets.QPushButton()
        self.init_button.setText("Initialize Syringe")
        self.init_button.clicked.connect(self.handleInitializeSyringe)

        self.command_control_label = QtWidgets.QLabel()
        self.command_control_label.setText("Send Hamilton Command")
        self.command_control_entry_box = QtWidgets.QLineEdit()
        self.command_control_entry_box.setText("")
        self.command_control_entry_box.editingFinished.connect(self.setCommand)
        self.command_button = QtWidgets.QPushButton()
        self.command_button.setText("Send Command")
        self.command_button.clicked.connect(self.sendCommandFromCommandBuffer)

        self.check_running_status_button = QtWidgets.QPushButton()
        self.check_running_status_button.setText('Check Status')
        self.check_running_status_button.clicked.connect(self.checkStatus)

        self.move_log_slot_button = QtWidgets.QPushButton()
        self.move_log_slot_button.setText('Move Logical Slot')
        self.move_log_slot_button.clicked.connect(self.moveToLogicalSlot)

        self.query_logical_pos_button = QtWidgets.QPushButton()
        self.query_logical_pos_button.setText('Query Logical Position')
        self.query_logical_pos_button.clicked.connect(self.queryLogicalPos)
        ...
        self.mainWidgetLayout.addWidget(self.init_button)
        self.mainWidgetLayout.addWidget(self.command_control_label)
        self.mainWidgetLayout.addWidget(self.command_control_entry_box)
        self.mainWidgetLayout.addWidget(self.command_button)

        self.mainWidgetLayout.addWidget(self.check_running_status_button)
        self.mainWidgetLayout.addWidget(self.move_log_slot_button)
        self.mainWidgetLayout.addWidget(self.query_logical_pos_button)
    ...
    def checkStatus(self):
        self.pump.checkStatus()

    def moveToLogicalSlot(self):
        self.pump.moveToLogicalSlot()

    def queryLogicalPos(self):
        self.pump.queryLogicalPos()

    def setCommand(self):
        command_text = self.command_control_entry_box.displayText()
        print(f'Raw Command to Send: {repr(command_text)}')

    def sendCommandFromCommandBuffer(self):
#        response = self.pump.sendStringAndWaitUntilIdle(self.command_control_entry_box.displayText())
        response = self.pump.sendString(f'/1{self.command_control_entry_box.displayText()}\r')
    ...
    def handleInitializeSyringe(self):
        self.pump.initializeSyringe()
        time.sleep(0.1)
        self.pollPumpStatus()
